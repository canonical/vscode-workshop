...
/**
 * Run a refresh in `wait-on-error`/`continue`/`abort` mode and notify on
 * completion. Records a session when it actually connected into the workshop —
 * a paused-and-dismissed, aborted, or `reopen: false` run stays local.
 *
 * Callers are responsible for building `callbacks` (e.g. via `refreshCallbacks`)
 * and for handling success/failure (session writes, error display).
 */
export function runResumeRefresh(
  client: WorkshopClient,
  projectPath: string,
  workshop: Workshop,
  callbacks: ReopenCallbacks,
  ...
  refreshAndReopen(client, projectPath, workshop, callbacks, mode, reopen)
    .then((hostname) => {
      if (hostname) {
